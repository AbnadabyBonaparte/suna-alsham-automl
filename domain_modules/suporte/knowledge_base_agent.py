"""
ALSHAM QUANTUM - Support Knowledge Base Agent
Agente especializado em base de conhecimento inteligente para suporte
Versão: 2.0 - Implementação nativa com base de conhecimento completa
"""
...
class KnowledgeBaseAgent(BaseNetworkAgent):
    ...
    def __init__(self):
        super().__init__(
            agent_id="support_knowledge_base",
...
def create_agents() -> List[BaseNetworkAgent]:
    ...
    """
    try:
        # Cria instância do agente knowledge base
        knowledge_base_agent = KnowledgeBaseAgent()
        
        logger.info("✅ Support Knowledge Base Agent criado com sucesso")
        
        return [knowledge_base_agent]
        
    except Exception as e:
        logger.error(f"❌ Erro ao criar Support Knowledge Base Agent: {str(e)}")
        return []

# Teste standalone
if __name__ == "__main__":
    async def test_knowledge_base_agent():
        """Teste completo do agente knowledge base"""
        print("🧪 Testando Support Knowledge Base Agent...")
        ...
        print(f"📚 Base carregada: {len(agent.knowledge_base)} artigos")
        
        # Teste 1: Busca por senha
        print("\n🔍 Teste 1: Busca por 'senha'...")
        ...
        if result['status'] == 'success':
            ...
            articles = response['found_articles']
            print(f"  • Artigos encontrados: {response['total_found']}")
            for article in articles[:2]:
                print(f"    - {article['title']} (Score: {article['relevance_score']:.2f})")
            print(f"  • Tempo de busca: {response['search_time']}")
        
        # Teste 2: Busca por categoria
        print("\n🏷️ Teste 2: Busca na categoria 'Billing'...")
        ...
        if result['status'] == 'success':
            response = result['response']
            print(f"  • Artigos na categoria: {response['total_found']}")
            print(f"  • Filtro aplicado: {response['category_filter']}")
            for article in response['found_articles']:
                print(f"    - {article['title']} (Categoria: {article['category']})")
        
        # Teste 3: Recuperar artigo específico
        print("\n📄 Teste 3: Recuperar artigo específico...")
        ...
        if result['status'] == 'success':
            ...
            print(f"  • Passos: {len(article['steps'])} etapas")
            print(f"  • Popularidade: {article['popularity']}")
        
        # Teste 4: Listar categorias
        print("\n📋 Teste 4: Categorias disponíveis...")
        
        message = {"action": "get_categories"}
        result = await agent._internal_handle_message(message)
        
        if result['status'] == 'success':
            response = result['response']
            categories = response['categories']
            print(f"  • Total de categorias: {response['total_categories']}")
            for category, info in categories.items():
                print(f"    - {category}: {info['total_articles']} artigos")
        
        # Teste 5: Artigos populares
        print("\n🔥 Teste 5: Artigos populares...")
        
        message = {
            "action": "get_popular_articles",
            "data": {
                "limit": 5
            }
        }
        ...
        if result['status'] == 'success':
            response = result['response']
            popular = response['popular_articles']
            print(f"  • Artigos populares: {len(popular)}")
            for article in popular:
                print(f"    - {article['title']} (Views: {article['popularity']})")
        
        # Teste 6: Sugestões
        print("\n💡 Teste 6: Sugestões para 'senha'...")
        ...
        if result['status'] == 'success':
            ...
            print(f"  • Sugestões encontradas: {response['total_suggestions']}")
            for suggestion in suggestions:
                print(f"    - {suggestion['text']} (Tipo: {suggestion['type']})")
        
        # Teste 7: Adicionar novo artigo
        print("\n➕ Teste 7: Adicionar novo artigo...")
        
        message = {
            "action": "add_article",
            "data": {
                "title": "Como usar 2FA (Autenticação em Duas Etapas)",
                "content": "A autenticação em duas etapas adiciona uma camada extra de segurança à sua conta.",
                "category": "Security",
                "tags": ["2fa", "segurança", "autenticação", "proteção"],
                "steps": [
        ...
        if result['status'] == 'success':
            response = result['response']
            print(f"  • Artigo adicionado: ID {response['article_id']}")
            print(f"  • Total de artigos: {response['total_articles']}")
        
        # Teste 8: Status da base
        print("\n📊 Teste 8: Status da knowledge base...")
        
        message = {"action": "get_kb_status"}
        result = await agent._internal_handle_message(message)
        
        if result['status'] == 'success':
            response = result['response']
            kb_stats = response['knowledge_base_stats']
            search_stats = response['search_statistics']
            performance = response['performance_metrics']
            
            print(f"  • Total de artigos: {kb_stats['total_articles']}")
            print(f"  • Categorias: {kb_stats['total_categories']}")
            print(f"  • Tags: {kb_stats['total_tags']}")
            print(f"  • Buscas realizadas: {search_stats['total_searches']}")
            print(f"  • Taxa de sucesso: {performance['success_rate']}")
            print(f"  • Tempo médio de busca: {performance['avg_search_time']}")
            print(f"  • Média de resultados: {performance['avg_results_per_search']}")
        
        print(f"\n✅ Todos os testes concluídos! Agente funcionando perfeitamente.")
        print(f"🎯 Support Knowledge Base Agent - Status: OPERACIONAL")
