...
class CustomerSuccessAgent(BaseNetworkAgent):
    ...
    async def _handle_ai_response(self, response_message: AgentMessage) -> None:
        """
        Handles the response containing risk analysis from the AIPoweredAgent.

        This method validates the assessment context, checks the completion status,
        ...
        Args:
            response_message (AgentMessage): The message containing the AI risk analysis response.

        Returns:
            None
        """
        assessment_id: str = response_message.callback_id
        if assessment_id not in self.pending_assessments:
            logger.warning(f"[CustomerSuccessAgent] Assessment ID '{assessment_id}' não encontrado em pending_assessments.")
            return

        task_context: Dict[str, Any] = self.pending_assessments.pop(assessment_id)
        original_message: AgentMessage = task_context["original_message"]

        if response_message.content.get("status") != "completed":
            logger.error(f"[CustomerSuccessAgent] Avaliação de risco pela IA falhou para a tarefa {assessment_id}.")
            await self.publish_error_response(original_message, "Falha na avaliação de risco pela IA.")
            return

        risk_analysis: Dict[str, Any] = response_message.content.get("result", {}).get("structured_data", {})
        logger.info(f"[CustomerSuccessAgent] Análise de risco recebida da IA para a tarefa {assessment_id}: {risk_analysis}")
