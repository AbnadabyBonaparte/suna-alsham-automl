...
export default function LandingPage() {
    ...
    const render = () => {
      ...
      stars.forEach((star) => {
        ...
        if (isEntering || speed > 10) {
          ...
          ctx.strokeStyle = `${primaryClr}${Math.round(alpha * 255)
            .toString(16)
            .padStart(2, '0')}`;
          ctx.lineWidth = size;
          ctx.beginPath();
      ...
      if (isEntering && speed > 40) {
        ctx.fillStyle = `${textClr}${Math.round(((speed - 40) / 10) * 255)
          .toString(16)
          .padStart(2, '0')}`;
        ctx.fillRect(0, 0, w, h);
      }
  ...
  return (
    ...
    >
      ...
      {/* CONTEÚDO CENTRAL */}
      <div
        className={`relative z-10 h-screen flex flex-col items-center justify-center transition-opacity duration-500 ${isEntering ? 'opacity-0' : 'opacity-100'}`}
      >
        {/* Logo Hero */}
        <div className="text-center mb-12 space-y-4 cursor-default">
          ...
          <h1 className="text-6xl md:text-9xl font-black text-text tracking-tighter font-display mix-blend-difference animate-scaleIn">
            ALSHAM
            <span className="block text-transparent bg-clip-text bg-gradient-to-b from-text to-textSecondary">
              QUANTUM
            </span>
          </h1>
          ...
          <p className="text-textSecondary max-w-md mx-auto font-light text-sm md:text-base tracking-wide leading-relaxed animate-fadeInUp">
            A convergência final entre inteligência artificial e controle operacional. Bem-vindo à
            Singularidade.
          </p>
        </div>
      ...
      {/* RODAPÉ */}
      <div
        className={`absolute bottom-8 w-full text-center transition-opacity duration-500 ${isEntering ? 'opacity-0' : 'opacity-100'}`}
      >
