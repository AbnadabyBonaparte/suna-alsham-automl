...
export default function AdminPage() {
  ...
  const { users, loading, totalUsers } = useAdmin();
  ...
  useEffect(() => {
    ...
    // Reflete o número real de usuários (sem inflar com "almas" fictícias)
    const USER_COUNT = Math.min(users.length, 50);
        ...
        <div className="lg:col-span-2 bg-background/60 backdrop-blur-xl border border-border/10 rounded-3xl overflow-hidden relative h-64 group shadow-2xl">
          <div className="absolute top-6 left-6 z-10">
            ...
            <p className="text-xs text-textSecondary font-mono uppercase">
              {totalUsers > 0
                ? `${totalUsers} Total Registered • Active: —`
                : 'No registered users'}
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl border border-border/5 bg-surface/5">
              ...
              <button
                ...
                style={{
                  background: dbStatus === 'FROZEN' ? 'var(--color-error)' : 'var(--color-border)',
                }}
              >
                <div
                  className={`absolute top-1 left-1 w-4 h-4 bg-surface rounded-full transition-transform duration-300 ${dbStatus === 'FROZEN' ? 'translate-x-6' : ''}`}
                />
              </button>
            </div>
            ...
            <div className="relative group">
              ...
              {!isSafetyOff && (
                ...
                >
                  ...
                  <span className="text-xs font-mono font-bold text-textSecondary tracking-widest">
                    DISENGAGE SAFETY
                  </span>
                </button>
              )}
        ...
        <div className="bg-background border border-border/10 rounded-3xl p-6 flex flex-col shadow-xl">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-text text-sm tracking-wider">
                REALITY EDITOR (SQL)
              </span>
            </div>
            <span
              ...
              style={{
                background:
                  dbStatus === 'OPTIMAL' ? 'var(--color-success)/20' : 'var(--color-error)/20',
                color: dbStatus === 'OPTIMAL' ? 'var(--color-success)' : 'var(--color-error)',
                border: `1px solid ${dbStatus === 'OPTIMAL' ? 'var(--color-success)/30' : 'var(--color-error)/30'}`,
              }}
            >
              DB_STATUS: {dbStatus}
            </span>
        ...
        <div className="bg-background/40 backdrop-blur-xl border border-border/10 rounded-3xl p-6 flex flex-col">
          ...
          <div className="space-y-2 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-border/10">
            ...
            {filteredUsers.slice(0, 20).map((user) => (
              <div
                key={user.id}
                className="group flex items-center justify-between p-3 rounded-xl bg-surface/5 hover:bg-surface/10 border border-transparent hover:border-border/10 transition-all cursor-pointer"
              >
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    className="p-2 hover:bg-[var(--color-error)]/20 rounded-lg transition-colors"
                    style={{ color: 'var(--color-error)' }}
                  >
                    <Trash2 className="w-3 h-3" />
                  </button>
                  <button
                    className="p-2 hover:bg-[var(--color-primary)]/20 rounded-lg transition-colors"
                    style={{ color: 'var(--color-primary)' }}
                  >
