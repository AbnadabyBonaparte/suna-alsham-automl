...
import { Activity, Shield, Zap, Database, Terminal, Cpu, PauseOctagon, PlayCircle } from "lucide-react";
import { useQuantumStore } from "@/lib/store";
import { toggleSystemMode } from "@/lib/actions"; // Importamos a Server Action
import { useSfx } from "@/hooks/use-sfx"; // Importamos os sons
...
export default function DashboardPage() {
  ...
  // Função para disparar comandos reais ao Supabase
  const handleCommand = async (mode: 'TURBO' | 'STOP') => {
    setIsPending(true);
    play(mode === 'STOP' ? 'alert' : 'click'); // Som tático

    try {
      const result = await toggleSystemMode(mode);
      if (result.success) {
        play('ambient'); // Feedback de sucesso
        console.log(`[SUNA-CORE] Modo ${mode} ativado via Server Action.`);
      } else {
        play('alert');
        console.error("Falha no comando:", result.error);
      }
    } catch (error) {
      console.error("Erro crítico de comando:", error);
    } finally {
      setIsPending(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6 bg-black/20 backdrop-blur-sm sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          <Button 
            ...
            className="bg-purple-600/20 hover:bg-purple-600/40 text-purple-300 border border-purple-500/30 transition-all"
          >
          ...
          <Button 
            ...
            className="bg-red-900/20 hover:bg-red-900/40 text-red-400 border border-red-500/30 transition-all"
          >
            <PauseOctagon className="mr-2 h-4 w-4" />
            STOP SYSTEM
          </Button>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {agents.map((agent) => (
            <Card 
              key={agent.id} 
              className="bg-zinc-900/40 border-white/5 backdrop-blur-sm hover:border-purple-500/50 hover:bg-zinc-900/60 transition-all duration-300 group relative overflow-hidden"
            >
              ...
              <CardContent>
                <div className="flex items-end justify-between mb-2">
                   <div className="text-2xl font-bold text-white font-mono">
                     {agent.efficiency ? agent.efficiency.toFixed(0) : 0}%
                   </div>
                ...
                <div className="w-full bg-black/50 h-1.5 rounded-full overflow-hidden">
                    <div 
                        className={`h-full transition-all duration-1000 ease-in-out ${getBarColor(agent.efficiency || 0)}`}
...
function StatusBadge({ status }: { status: string }) {
    const colors: Record<string, string> = {
        ACTIVE: "bg-green-500/10 text-green-400 border-green-500/20", // Adicionei ACTIVE explicitamente
        ...
        CRITICAL: "bg-red-900/20 text-red-500 border-red-500/50 animate-bounce", // Novo status crítico
    };
    // Fallback para IDLE se o status vier desconhecido
    return (
