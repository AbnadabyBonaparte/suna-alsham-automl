      ...
      try {
        ...
        const networkServers: NetworkServer[] = [
          ...
          },
          {
            ...
            name: 'ORION Core',
            type: 'agent',
            load: Math.floor(Math.random() * 50 + 40),
            status: 'online',
            requests: Math.floor((requestsCount || 0) * 0.3),
          },
          {
            id: 'VOID-HUB',
            lat: 51,
            ...
            status: 'online',
            requests: Math.floor((requestsCount || 0) * 0.2),
          },
          {
            id: 'NEXUS-CORE',
            lat: 35,
            ...
            status: 'online',
            requests: Math.floor((requestsCount || 0) * 0.25),
          },
          {
            id: 'USER-PORTAL',
            lat: -23,
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col lg:flex-row gap-6 p-2 overflow-hidden relative">
      ...
      <div className="w-full lg:w-80 flex flex-col gap-4 z-10 h-full pointer-events-none">
        <div className="bg-background/60 backdrop-blur-xl border border-[var(--color-primary)]/20 rounded-2xl p-6 pointer-events-auto shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            <Globe
              className="w-6 h-6 animate-spin"
              style={{ animationDuration: '10s', color: 'var(--color-primary)' }}
            />
            <div>
              <h1 className="text-xl font-black text-text tracking-tight font-display">
                GLOBAL NET
              </h1>
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-border/10">
            {servers.map((server) => (
              <div
                ...
                className="group p-3 rounded-lg border border-border/5 bg-surface/5 hover:bg-surface/10 hover:border-[var(--color-primary)]/50 transition-all cursor-pointer hover:scale-105"
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    {server.type === 'database' ? (
                      <Database className="w-3 h-3" style={{ color: 'var(--color-success)' }} />
                    ) : server.type === 'api' ? (
                      <Cpu className="w-3 h-3" style={{ color: 'var(--color-accent)' }} />
                    ) : server.type === 'agent' ? (
                      <Brain className="w-3 h-3" style={{ color: 'var(--color-warning)' }} />
                    ) : (
                  ...
                  <span
                    ...
                    style={{
                      color: server.load > 80 ? 'var(--color-error)' : 'var(--color-success)',
                    }}
                  >
                    {server.load}% LOAD
                  </span>
                ...
                <div className="mt-3 h-1 w-full bg-background/50 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${server.load}%`,
                      background: server.load > 80 ? 'var(--color-error)' : 'var(--color-primary)',
                    }}
                  />
                </div>
              </div>
      ...
      <div className="absolute top-6 right-6 z-10 flex flex-col gap-3 w-64">
        <div className="bg-background/60 backdrop-blur-xl border border-border/10 rounded-xl p-4">
          ...
          <div className="text-3xl font-mono text-text">
            {supabaseLatency} <span className="text-sm text-textSecondary">ms</span>
          </div>
        </div>
      ...
      {selectedServer && (
        ...
        >
          <div
            className="relative bg-background/95 border-2 border-[var(--color-primary)]/50 backdrop-blur-xl rounded-2xl p-8 max-w-2xl w-full shadow-[0_0_80px_rgba(6,182,212,0.3)]"
          ...
          >
            ...
            <div className="flex items-center gap-6 mb-8">
              <div
                className="p-6 rounded-2xl bg-background/80 border border-[var(--color-primary)]/30 shadow-[0_0_20px_var(--color-primary)/30]"
                style={{ color: 'var(--color-primary)' }}
              >
                {selectedServer.type === 'database' ? (
                  <Database className="w-8 h-8" />
                ) : selectedServer.type === 'api' ? (
                  <Cpu className="w-8 h-8" />
                ) : selectedServer.type === 'agent' ? (
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <div className="bg-background/40 border border-border/10 rounded-xl p-6">
                ...
                <div
                  ...
                  style={{
                    color: selectedServer.load > 80 ? 'var(--color-error)' : 'var(--color-success)',
                  }}
                >
                  {selectedServer.load}%
                </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-2 font-mono">
                    ...
                    <span
                      ...
                      style={{
                        color:
                          selectedServer.load > 80 ? 'var(--color-error)' : 'var(--color-primary)',
                      }}
                    >
                      {selectedServer.load}%
                    </span>
