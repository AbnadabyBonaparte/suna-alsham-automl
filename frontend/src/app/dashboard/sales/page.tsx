...
function DealModal({ deal, onClose }: DealModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-fadeIn">
      <div
        className={`relative w-full max-w-2xl ${colors.bg} backdrop-blur-xl border ${colors.border} rounded-3xl p-8 shadow-2xl`}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 text-xs">
              <span
                className={`px-2 py-1 rounded ${colors.bg} ${colors.text} border ${colors.border} uppercase font-bold`}
              >
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-surface/5 rounded-xl p-4">
            ...
            <div className="mt-2 h-2 w-full bg-background/50 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[var(--color-secondary)] to-[var(--color-primary)] transition-all"
        ...
        {deal.expected_close_date && (
          <div className="mb-6">
            <h3 className="text-sm font-bold text-textSecondary uppercase mb-2">
              Expected Close Date
            </h3>
      ...
      <div className="lg:w-2/3 w-full flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            {
              label: 'Total Revenue',
              val: `$${stats.total_value.toLocaleString()}`,
              icon: DollarSign,
              colorVar: '--color-success',
            },
            {
              label: 'Conversion Rate',
              val: `${stats.conversion_rate.toFixed(1)}%`,
              icon: Zap,
              colorVar: '--color-warning',
            },
            {
              label: 'Active Deals',
              val: stats.total_deals.toString(),
              icon: Users,
            ...
            <div
              key={i}
              className="bg-background/60 backdrop-blur-xl border border-border/10 rounded-2xl p-5 relative overflow-hidden group"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
              <div className="flex justify-between items-start mb-2">
                <div
                  className="p-2 rounded-lg bg-surface/5"
              ...
              <div className="text-xs text-textSecondary uppercase font-bold tracking-wider">
                {kpi.label}
              </div>
            </div>
          ))}
      ...
      <div className="lg:w-1/3 w-full flex flex-col gap-4 h-full">
        ...
        <div className="flex-1 bg-background/60 backdrop-blur-xl border border-border/10 rounded-3xl p-6 overflow-hidden flex flex-col">
          <div className="flex items-center justify-between mb-4">
            ...
            {deals.length > 0 && (
              <span
                className="text-[10px] px-2 py-1 rounded animate-pulse"
                style={{
        ...
        <div className="bg-background/60 backdrop-blur-xl border border-border/10 rounded-2xl p-4">
          <h3 className="text-xs font-bold text-textSecondary uppercase mb-3">
            Pipeline Breakdown
          </h3>
