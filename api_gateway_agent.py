#!/usr/bin/env python3
"""
API Gateway Agent - Gateway Inteligente para SUNA-ALSHAM System
Gerenciamento avançado de APIs, rate limiting, autenticação e roteamento
"""
...
class APIGatewayAgent(BaseNetworkAgent):
    """Agente de Gateway API Inteligente"""
    ...
    async def _log_request(self, request: Request, response, response_time: float, request_id: str):
        ...
        total_time = (self.gateway_metrics['average_response_time'] * 
                     (self.gateway_metrics['total_requests'] - 1) + response_time)
...
def create_api_gateway_agent(message_bus, num_instances=1) -> List[APIGatewayAgent]:
    """
    Cria agente de API Gateway
    ...
    Returns:
        Lista com 1 agente de API Gateway
    """
    agents = []
    
    try:
        logger.info("🌐 Criando APIGatewayAgent para SUNA-ALSHAM...")
        ...
        agent_id = "api_gateway_001"
        
        if agent_id not in existing_agents:
            try:
                agent = APIGatewayAgent(agent_id, AgentType.SERVICE, message_bus)
                
                # Iniciar serviços do gateway
                asyncio.create_task(agent.start_gateway_service())
                
                agents.append(agent)
                logger.info(f"✅ {agent_id} criado como Gateway Inteligente")
        ...
        logger.info(f"✅ {len(agents)} agente de API Gateway criado")
        
        return agents
        
    except Exception as e:
        logger.error(f"❌ Erro crítico criando APIGatewayAgent: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return []
