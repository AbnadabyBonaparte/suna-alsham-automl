...
class AnalyticsAgent(BaseNetworkAgent):
    ...
    def __init__(self, agent_id: str, message_bus):
        ...
        logger.info(
            f"✅ {self.agent_id} inicializado com especialização em {self.specialty.value}"
        )
...
AGENT_CONFIGURATIONS = [
    ...
    # Podemos adicionar mais agentes especializados aqui no futuro.
]
...
def create_specialized_agents(message_bus) -> List[BaseNetworkAgent]:
    ...
    logger.info(f"📋 {len(existing_agents)} agentes já existentes no sistema detectados.")
    ...
    for config in AGENT_CONFIGURATIONS:
        if config.agent_id not in existing_agents:
            ...
            logger.warning(
                f"⚠️ Agente {config.agent_id} já existe - pulando criação."
            )
