...
class TaskDelegatorAgent(BaseNetworkAgent):
    ...
    async def _handle_task_delegation(self, message: AgentMessage):
        ...
        # Lógica de delegação seria implementada aqui
        response = {
            "status": "delegated",
            "task_type": task_type,
            "assigned_agents": [],  # Lista de agentes que receberam subtarefas
    ...
    async def _handle_task_decomposition(self, message: AgentMessage):
        ...
        # Lógica de decomposição seria implementada aqui
        response = {
            "status": "decomposed",
            "subtasks": [],  # Lista de subtarefas criadas
...
class NewAgentOnboardingAgent(BaseNetworkAgent):
    ...
    def __init__(self, agent_id: str, message_bus):
        ...
        # Sistema de controle de detecções
        self.known_agents: Set[str] = set()
        self.last_detection_time: float = 0
        self.detection_debounce_seconds: float = 2.0  # Debounce de 2 segundos
        self.last_known_count: int = 0
        self.detection_history: List[Dict[str, Any]] = []
        
        # Inicializar tarefa de monitoramento
    ...
    async def _monitor_network_changes(self, interval: int = 15):
        """
        Monitora mudanças na rede com sistema de debounce inteligente.
        CORREÇÃO: Elimina detecções duplicadas e reduz spam de logs.
        """
        ...
        while True:
            try:
                ...
                if current_agents != self.known_agents:
                    ...
                    # Aplicar debounce: só processar se passou tempo suficiente desde última detecção
                    if current_time - self.last_detection_time >= self.detection_debounce_seconds:
                        
                        # Identificar novos agentes (não duplicatas)
                        ...
                        # Detecção muito próxima da anterior - ignorar (debounce)
                    ...
                    # Rede estável - log periódico de status
    ...
    async def _process_new_agents(self, new_agents: Set[str]):
        """
        Processa novos agentes detectados (sem duplicatas).
        """
        if not new_agents:
            return
        ...
        # Realizar onboarding para cada novo agente
    ...
    async def _process_removed_agents(self, removed_agents: Set[str]):
        """
        Processa agentes que saíram da rede.
        """
    ...
    async def _check_agent_health(self, agent_id: str) -> Dict[str, Any]:
        """
        Verifica a saúde de um agente específico.
        """
        # Simulação de verificação de saúde
        # Em implementação real, enviaria mensagem de ping para o agente
        return {
            "status": "healthy",
            "response_time": 0.05,
            "last_seen": time.time()
    ...
    async def _discover_agent_capabilities(self, agent_id: str) -> List[str]:
        """
        Descobre as capacidades de um agente específico.
        """
        # Simulação de descoberta de capacidades
        # Em implementação real, consultaria o agente sobre suas capabilities
        basic_capabilities = ["message_handling", "async_processing"]
        
        # Inferir capacidades baseado no ID (heurística simples)
    ...
    async def _internal_handle_message(self, message: AgentMessage):
        """Processa mensagens relacionadas ao onboarding"""
        if message.message_type == MessageType.REQUEST:
            request_type = message.content.get("request_type")
    ...
    async def _handle_network_status_request(self, message: AgentMessage):
        ...
        response = {
            ...
            "recent_detections": len([h for h in self.detection_history if time.time() - h["timestamp"] < 300])  # Últimos 5 minutos
        }
        
        await self.publish_response(message, response)

    async def _handle_force_rescan(self, message: AgentMessage):
        """Força um novo scan da rede"""
        logger.info("🔄 Forçando novo scan da rede...")
        
        # Reset do estado para forçar nova detecção
    ...
    async def _handle_history_request(self, message: AgentMessage):
        ...
        response = {
            "status": "completed",
            "detection_history": self.detection_history[-10:],  # Últimas 10 detecções
    ...
    def __del__(self):
        """Cleanup quando o agente é destruído"""
...
def create_specialized_agents(message_bus) -> List[BaseNetworkAgent]:
    """
    Cria os agentes Especializados com sistema otimizado.
    """
