...
Versão Suprema – Carrega todas as plantas de agentes e inicializa a rede.
Robusto contra falhas e preparado para relatórios detalhados.
"""

import logging
from typing import Any, Dict, List

# 🔧 Corrige path para permitir imports mesmo em containers
import sys
from pathlib import Path
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root.resolve()))

# 🔗 Importa todas as fábricas de agentes
...
async def initialize_all_agents(network: Any) -> Dict[str, Any]:
    """
    Inicializa todos os agentes do sistema de forma segura e auditável.
    """
    agents_loaded = 0
    failed_modules: List[str] = []

    # 📜 Lista de todas as fábricas (plantas de agentes)
    ...
    for factory in agent_factories:
        ...
        try:
            ...
            for agent in agents:
                ...
                agents_loaded += 1
                logger.debug(f"✅ Agente registrado: {agent.agent_id}")
            logger.info(f"<-- SUCESSO: {factory_name} criou {len(agents)} agente(s).")
        except Exception as e:
            logger.error(f"❌ FALHA: {factory_name} não conseguiu criar agentes: {e}", exc_info=True)
