...
def create_service_agents(message_bus, num_instances=1) -> List:
    ...
    try:
        for i in range(num_instances):
            agents.extend([
                CommunicationAgent(f"communication_{(i*2+1):03d}", AgentType.SERVICE, message_bus),
                DecisionAgent(f"decision_{(i*2+1):03d}", AgentType.SERVICE, message_bus)
            ])
        # Limitar a 2 agentes no total
        agents = agents[:2]
        for agent in agents:
